...
<?php if ($not_skeleton) :
	...
	if (tribe_get_venue_id()) {
		if (! $set_venue_apart && ! tribe_embed_google_map()) {
			$meta_class .= ' bt-meta-style'; 
		} elseif (! $set_venue_apart && ! tribe_has_organizer() && tribe_embed_google_map()) {
			$meta_class .= ' bt-meta-style-2'; 
		} else {
			$meta_class .= ' bt-meta-style-3'; 
		}
	}else{
		$meta_class .= ' bt-meta-style-4'; 
	}
	echo '<div class="tribe-events-single-section tribe-events-event-meta primary tribe-clearfix'.$meta_class.'">';
 endif; ?>
